    ...
    private void drawDebug(int[] xPoints, int[] yPoints, LightSource ls)
    {
        ...
        for (int i = 0; i < xPoints.length; i++) {
            ...
            if (i==0) {
                for (int x = -1; x <= 1;x++) {
                    for (int y = -1; y <= 1; y++) {
                        camImg.drawLine(xPoints[i]+x, yPoints[i]+y, xPoints[xPoints.length-1]+x, yPoints[yPoints.length-1]+y);
                    }
                }
            } else {
                for (int x = -1; x <= 1;x++) {
                    for (int y = -1; y <= 1; y++) {
                        camImg.drawLine(xPoints[i]+x, yPoints[i]+y, xPoints[i-1]+x, yPoints[i-1]+y);
                    }
                }
            }
