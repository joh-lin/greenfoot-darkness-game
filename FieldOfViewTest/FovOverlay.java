...
public class FovOverlay extends Actor
{
    ...
    private int lightRes = 4;   // determines how many pixels to skip (bigger = faster) (default=4)
    ...
    private float smoothingRadius = 10;
    ...
    {
    	GreenfootImage imageCopy = new GreenfootImage(image);
    			...
    			for (int x2 = -1; x2 < 2; x2++) {
    				for (int y2 = -1; y2 < 2; y2++) {
    					if ((x+x2) > 0 && (x+x2) < w && (y+y2) > 0 && (y+y2) < h) {
    						c++;
    						alpha += imageCopy.getColorAt(x+x2, y+y2).getAlpha();
    					}
    				}
    			}
    			Color color = new Color(0, 0, 0, (int)(alpha/c));
